...
void TAREA_PRINCIPAL_Tasks ( void )
{
    while (1)
    {
        ...
        { 
            ...
            {
                xTaskCreate((TaskFunction_t) TAREA_Can1, "TAREA_Can1", 512, NULL, 2, &xTAREA_Can1); //Creo tarea para envio trama 1 por can
            }

            if (readByte == '2')                               //Si el dato recibido es el caracter 2
            {
                xTaskCreate((TaskFunction_t) TAREA_Can2, "TAREA_Can2", 512, NULL, 2, &xTAREA_Can2); //Creo tarea para envio trama 2 por can
            }
