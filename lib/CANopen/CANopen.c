...
/*=====================[Variables]================================*/
    ...
    struct k_sem canMutexLock;                                                   //Semaforo para proteger el periferico can ya que es un recurso compartico con otros hilos o tareas
...
uint8_t CANopen_init(void){
    ...
    int resultado = k_sem_init(&canMutexLock, 0, K_SEM_MAX_LIMIT);               //Creo e inicializo semaforo para proteger el recurso compartido de can con otras tareas
   ...
   if( resultado != 0)                                                           //Si no se creo el semaforo
    {
        ...
        k_sem_give(&canMutexLock);                                              //Arranco con semaforo liberado
    }
  
    mcan_fd_interrupt_config();                             //Configuro el periferico can
...
uint8_t Verify_writing_by_can(void){
    ...
    while(true){                                              //Bloque para verificar envio correcto por periferico can. Posee timeout
        ...
        k_msleep(1);                                          //Deje que la tarea quede inactiva por un tiempo determinado dejando que se produzca el cambio de contexto a otra tarea.
    }       
...
uint8_t CANopen_SDO_Expedited_Write(uint8_t node_id, uint8_t command, uint16_t index, uint8_t subindex,  uint8_t *data, uint8_t mode, uint8_t data_len){
    if(state==CANopen_PRE_OPERATIONAL || state==CANopen_OPERATIONAL){        //Si el estado del CANopen permite mensajes SDO
        k_sem_take(&canMutexLock, K_FOREVER);                                //Tomo semaforo para proteger el bus can ya que es un recurso compartico con otras tareas
        static uint8_t message[16] = {0}; 
        ...
        MCAN_MODE MCAN_MODES=MCAN_MODE_NORMAL;
        
        if(data_len>4){
            MCAN_MODES=MCAN_MODE_FD_STANDARD;
        }
        
        //Envio mensaje por can
        bool retorno = mcan_fd_interrupt_enviar((uint32_t) id, message, data_len+4, MCAN_MODES); //Envio trama por can bus
        if ( retorno == false){
            mcan_fd_interrupt_habilitar();                          //Habilito mcan para seguir operando en el proximo mensaje sdo
            k_sem_give(&canMutexLock);                              //Libero semaforo
            return 1;
        }
                    ...
                    case CAN_RECEPCION_OK:                        //Si se recibio mensaje                              
                    {
                        ...
                        if(rx_message[0]==0x60||rx_message[0]==0x43||rx_message[0]==0x4B||rx_message[0]==0x4F){ //Si la respuesta es un comando de respuesta
                            if(rx_message[0]==0x60){               //Si el mensaje es de confirmacion de escritura
                                ...
                                if((rx_message[1]==(index & 0xFF)) && (rx_message[2]==(index >> 8)) && (rx_message[3]==subindex)){
...
uint8_t CANopen_SDO_Expedited_Read(uint16_t *index, uint8_t *subindex){
    ...
    if(state==CANopen_PRE_OPERATIONAL || state==CANopen_OPERATIONAL){         //Si el estado del CANopen permite mensajes SDO
        ...
        while (true){
                ...
                case CAN_RECEPCION_OK:                                    
                {
                    ...
                    if(rx_messageID>=0x601 && rx_messageID<=0x67F){     //Si el mensaje tiene cobid SDO Rx
                        if(rx_messageID==(0x600+CANopen_nodeid)){       //El mensaje esta dedicado a este nodo
                            ...
                            if(rx_message[0]==0x23){                    //Si el comando SDO es escritura de 4 bytes
                                *index=(rx_message[2]<<8)+rx_message[1];//Paso index
                                *subindex=rx_message[3];                //Paso subindex
                            ...
                            if(rx_message[0]==0x2B){                    //Si el comando SDO es escritura de 2 bytes
                                *index=(rx_message[2]<<8)+rx_message[1];//Paso index
                                *subindex=rx_message[3];                //Paso subindex
                            ...
                            if(rx_message[0]==0x2F){                    //Si el comando SDO es escritura de 1 byte
                                *index=(rx_message[2]<<8)+rx_message[1];//Paso index
                                *subindex=rx_message[3];                //Paso subindex
                                uint8_t data=rx_message[4];
                                CANopen_Write_Dictionary(*index, *subindex, data, 8); //Escribo diccionario de objetos
                                retornar = 0;                           //Indico que se retorna ok
                                break;                                  //Salgo del switch
                            }
                        }else{
            ...
            if(interacciones*1>=CANopen_RSDO_timeout){
                retornar = 2;                                           //Indico que se retorna error en mensaje de recepcion
                mcan_fd_interrupt_habilitar();                          //Habilito mcan para seguir operando en el proximo mensaje sdo
                break;                                                  //Salgo del while true
            }
...
bool CANopen_Write_Dictionary(uint16_t index, uint8_t subindex, uint32_t data, uint8_t dictionary){
    ...
    k_sched_unlock();                                       //Salgo de la seccion critica
    return retorno;
}
